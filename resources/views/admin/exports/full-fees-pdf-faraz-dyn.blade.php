...
<head>
  ...
  <title>Tutelage Study - Malaysia Fee Schedule</title>
  <style>
    .page-break {
...
<body>
  <div class="container-fluid">
    <div>
      <center>
        <h3 class="main-heading">TUTELAGE STUDY - MALAYSIA FEE STRUCTURE</h3>
      </center>
    </div>
    <br>
    @php
    ...
    @foreach ($fees as $row)
      @php
        $otherfees = UniversityOtherFee::where('university_id', $row->university_id)
            ->where('level', $row->level)
            ->where('student_type', $row->student_type)
            ->first();
      @endphp
      <div class="card">
        <div class="card-header">
          <span class="float-left" style="display: inline-block">
            <span class="fw1 fs1" style="color: #753232">{{ strtoupper($row->getProgram->course_name) }} FEE
              SCHEDULE</span> <small class="fw2 fs2">({{ $row->student_type }})
            </small>
            <br>
            <small class="fw1 fs1" style="color: rgb(17, 91, 62);">{{ $row->getUniversity->name }} </small> (<small
              class="fw1 fs1" style="color: rgb(147, 0, 0);">{{ $row->level }}</small>)
          </span>
        </div>
        <div class="card-body">
